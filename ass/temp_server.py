from temp import receive_message, authenticate, user_exists_On9clients, user_exists_Off9clients, user_exists, user_blocked_list, user_blocked, length_encoded_msg
import datetime
import time
import sys
from select import *
...
while (1):
    ...
    #             print('\nConnection timeout for: {} since {}'.format(online_clients[notified_socket]['data'].decode().split(',')[
    #                   0], (online_clients[notified_socket]['last-active'] + datetime.timedelta(seconds=timeout)).strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]))
    #             server = 'WICKWICK\'S SERVER'.encode()
    #             server_header = f"{len(server):<{20}}".encode()
    #             message = 'Connection timeout at {} due to inactivity. Bye {}!'.format((online_clients[notified_socket]['last-active'] + datetime.timedelta(
    #                 seconds=timeout)).strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3], online_clients[notified_socket]['data'].decode()).encode()
    #             message_header = f"{len(message):<{20}}".encode()
    #             notified_socket.send(
    #                 server_header + server + message_header + message)
            ...
            while (1):
                ...
                for blocked in blocked_clients:
                    if blocked_clients[blocked]['data'].decode() == credentials[0]:
                        ...
                        current_time = datetime.datetime.now()
                        minus_blocked = current_time - \
                            datetime.timedelta(seconds=block_duration)
                        if minus_blocked > blocked_clients[blocked]['account-blocked'] or minus_blocked == blocked_clients[blocked]['account-blocked']:
                            ...
                            if username == credentials[0]:
                                message = 'Your account have been unblocked since {}, {}!'.format(
                                    current_time.strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3], username).encode()
                                message_header = f"{len(message):<{20}}".encode(
                                )
                            ...
                            if username == credentials[0]:
                                ...
                                message = 'Your account is blocked {}. Please try again after {}!'.format(
                                    username, (blocked_clients[blocked]['account-blocked'] + datetime.timedelta(seconds=block_duration)).strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
                                message_header = f"{len(message):<{20}}".encode(
                                )
                                client_socket.send(message_header + message)
                                break
                ...
                if 'Successful' in result:
                    ...
                    print('Accepted new connection from {}:{} ~> {}'.format(
                        *client_address, user['data'].decode()))

                    # send login date time
                    message = 'Welcome back {}! You Logged in at: {}'.format(user['data'].decode(
                    ), user['logged-in'].strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    client_socket.send(message_header + message)
                    ...
                    #                     message = '{} > {}'.format(
                    #                         msg['sender'].decode(), msg['message'].decode()).encode()
                    #                     message_header = f"{len(message):<{20}}".encode(
                    #                     )
                    #                     client_socket.send(
                    ...
                    #                 continue
                    #         message = '{} Logged in at {}'.format(user['data'].decode(
                    #         ), user['logged-in'].strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
                    #         message_header = f"{len(message):<{20}}".encode()
                    #         online_socket.send(
                    #             user['header'] + user['data'] + message_header + message)

                    message = 'offline messages successful, {}'.format(
                        user['data'].decode()).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    client_socket.send(message_header + message)
                    break
                    ...
                    if retry_count >= 3:
                        ...
                        message = 'Invalid Password. Retry count limit reached. Please try again later after {}. {}, Your account has been blocked!'.format(
                            (user['account-blocked'] + datetime.timedelta(seconds=block_duration)).strftime("%H:%M:%S.%f")[:-3], credentials[0]).encode()
                        message_header = f"{len(message):<{20}}".encode()
                        client_socket.send(message_header + message)

                    else:
                        result = result + \
                            ' retry count: {}'.format(retry_count)
        ...
        # server will reuse message header sent by sender for certain commands like message, broadcasting, etc
            ...
            if message is False:
                print('Closed connection from: {}'.format(
                    online_clients[notified_socket]['data'].decode()))
            ...
            print('Received message at {} from {}: {}'.format(user['last-active'].strftime(
                "%d/%m/%Y, %H:%M:%S.%f")[:-3], user["data"].decode(), message["data"].decode()))
            ...
            if command == 'broadcast':
                if length_encoded_msg(encoded_msg=message['data']) >= 2:
                    ...
                    message['data'] = message['data'].decode().split(' ', 1)[
                        1].encode()
                    ...
                    if blocked:
                        ...
                        notified_socket.send(
                            user['header'] + user['data'] + message_header + message)
                    # successful
                    message = 'broadcast successful, {}!'.format(
                        user['data'].decode()).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(
                        user['header'] + user['data'] + message_header + message)
                    ...
                    message = 'Error, No message to broadcast, {}'.format(
                        user['data'].decode()).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(
                        user['header'] + user['data'] + message_header + message)
            ...
            #             message = 'whoelse successful, {}!'.format(
            #                 user['data'].decode()).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #         message = 'whoelse {}'.format(
            #             len(list(online_clients))).encode()
            #         message_header = f"{len(message):<{20}}".encode()
            #         notified_socket.send(
            #             user['header'] + user['data'] + message_header + message)
            ...
            #                     online_clients[client_socket]['data'].decode()).encode()
            #                 message_header = f"{len(message):<{20}}".encode()
            #                 notified_socket.send(
            #                     user['header'] + user['data'] + message_header + message)
            #         # successful
            ...
            #         message = 'Error, whoelse need no args, {}'.format(
            #             user['data'].decode().split(',')[0]).encode()
            #         message_header = f"{len(message):<{20}}".encode()
            #         notified_socket.send(
            #             user['header'] + user['data'] + message_header + message)
            # # --- whoelse end ---
            ...
            #             message = 'whoelsesince successful, {}!'.format(
            #                 user['data'].decode()).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #             logged_in_time = online_clients[client_socket]['logged-in'] + datetime.timedelta(
            #                 seconds=sec)
            ...
            #                     msg = '{} IS online, last active: {}!'.format(online_clients[client_socket]['data'].decode(
            #                     ), online_clients[client_socket]['last-active'].strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
            #                     message_header = f"{len(msg):<{20}}".encode()
            #                     notified_socket.send(
            #                         user['header'] + user['data'] + message_header + msg)
            #         # checks offline user list
            #         for client_socket in offline_clients:
            #             current_time = datetime.datetime.now()
            #             sec = int(message['data'].decode().split(' ')[1])

            #             logged_in_time = offline_clients[client_socket]['logged-in'] + datetime.timedelta(
            #                 seconds=sec)
            ...
            #                 # if user's logged in time + <seconds> more than or equals to current time diplay user
            #                 if logged_in_time > current_time or logged_in_time == current_time:
            #                     msg = '{} WAS online, last active: {}!'.format(offline_clients[client_socket]['data'].decode(
            #                     ), offline_clients[client_socket]['last-active'].strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
            #                     message_header = f"{len(msg):<{20}}".encode()
            #                     notified_socket.send(
            #                         user['header'] + user['data'] + message_header + msg)
            #         # successful
            ...
            #         message = 'Error, Insufficient Args or Too Many Args, {}'.format(
            #             online_clients[notified_socket]['data'].decode().split(',')[0]).encode()
            #         message_header = f"{len(message):<{20}}".encode()
            #         notified_socket.send(
            #             user['header'] + user['data'] + message_header + message)
            # # --- whoelsesince end ---
            ...
            #         message = 'Error, Insufficient Args, {}!'.format(
            #             user['data'].decode().split(',')[0]).encode()
            #         message_header = f"{len(message):<{20}}".encode()
            #         notified_socket.send(
            #             user['header'] + user['data'] + message_header + message)
            ...
            #             message = 'Error, can\'t message self: {}!'.format(
            #                 recipient).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #                             message = 'Error, can\'t send message to: {}! *{} blocked you...'.format(
            ...
            #                                 user['header'] + user['data'] + message_header + msg)
            #                             # successful
            #                             message = 'online message successful, {}!'.format(
            #                                 user['data'].decode()).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            ...
            #                                 offline_messages[client_socket].append(
            #                                     {'sender_header': user['header'], 'sender': user['data'], 'recipient': offline_clients[client_socket]['data'], 'message': msg})
            #                             else:
            #                                 offline_messages[client_socket] = [
            #                                     {'sender_header': user['header'], 'sender': user['data'], 'recipient': offline_clients[client_socket]['data'], 'message':msg}]
            #                             # FIXME uncomment debugging print before submitting
            #                             # print('off9 MESSAGE sent to {} from {}'.format(recipient, user['data'].decode().split(',')[0]))
            #                             message = 'sent offline message to: {} successful!'.format(
            #                                 recipient).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            #                                 user['header'] + user['data'] + message_header + message)
            #                             # successful
            #                             message = 'offline message successful, {}!'.format(
            #                                 user['data'].decode()).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            ...
            #                             message = 'Error, can\'t send offline message to: {}! *{} blocked you...'.format(
            #                                 recipient, recipient).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            ...
            #                 message = 'Error, {} doesn\'t exist!'.format(
            #                     recipient).encode()
            #                 message_header = f"{len(message):<{20}}".encode()
            #                 notified_socket.send(
            #                     user['header'] + user['data'] + message_header + message)
            ...
            #             message = 'Error, can\'t block self: {}!'.format(
            #                 username).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #                         # check if user in my block list
            #                         if user_blocked_list(my_socket=notified_socket, to_check_socket=client_socket, on9clients=online_clients):
            #                             message = 'Error, already blocked {}!'.format(
            #                                 username).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            ...
            #                         user['header'] + user['data'] + message_header + message)
            #                     # successful
            #                     message = 'block successful, {}!'.format(
            #                         user['data'].decode()).encode()
            #                     message_header = f"{len(message):<{20}}".encode(
            #                     )
            #                     notified_socket.send(
            ...
            #             message = 'Error, can\'t unblock self: {}!'.format(
            #                 username).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #                                 # successful
            #                                 message = 'unblock successful, {}!'.format(
            #                                     user['data'].decode()).encode()
            #                                 message_header = f"{len(message):<{20}}".encode(
            #                                 )
            #                                 notified_socket.send(
            ...
            #                         message = 'Error, fail to unblock {}!'.format(
            #                             username).encode()
            #                         message_header = f"{len(message):<{20}}".encode(
            #                         )
            #                         notified_socket.send(
            ...
            # --- logout start ---
                ...
                if length_encoded_msg(encoded_msg=message['data']) == 1:
                    ...
                    print('\nConnection close for: {} at {}\n'.format(
                        user['data'].decode(), current_time.strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]))
                    message = 'Logged out successful at {} Bye!'.format(
                        current_time.strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(
                        user['header'] + user['data'] + message_header + message)
                    ...
                    #             if user_blocked_list(my_socket=client_socket, to_check_socket=notified_socket, on9clients=online_clients):
                    #                 continue
                    #         message = '{} Logged out at {}'.format(user['data'].decode(
                    #         ), user['last-active'].strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]).encode()
                    #         message_header = f"{len(message):<{20}}".encode()
                    #         client_socket.send(
                    #             user['header'] + user['data'] + message_header + message)
                    ...
                    # successful
                    message = 'logout successful, {}!'.format(
                        user['data'].decode()).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(
                        user['header'] + user['data'] + message_header + message)
                    ...
                    message = 'Error, Too many Args: {}!'.format(
                        user['data'].decode().split(',')[0]).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(
                        user['header'] + user['data'] + message_header + message)
            ...
            #             # FIXME uncomment debugging print before submitting
            #             # print('{} can\'t UNBLOCK SELF!'.format(username))
            #             message = 'Error, can\'t startprivate self: {}!'.format(
            #                 username).encode()
            #             message_header = f"{len(message):<{20}}".encode()
            #             notified_socket.send(
            #                 user['header'] + user['data'] + message_header + message)
            ...
            #                             message = 'Waiting for {} to accept your connection...'.format(
            #                                 online_clients[client_socket]['data'].decode()).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            #                                 user['header'] + user['data'] + message_header + message)
            #                             message = 'startprivate {} {} {} {}'.format(
            #                                 online_clients[client_socket]['private-connection'][0], online_clients[client_socket]['private-connection'][1], user['data'].decode(), online_clients[client_socket]['data'].decode()).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             notified_socket.send(
            #                                 user['header'] + user['data'] + message_header + message)
            ...
            #                             message = 'startprivate {} {} {} {}'.format(
            #                                 user['private-connection'][0], user['private-connection'][1], online_clients[client_socket]['data'].decode(), user['data'].decode()).encode()
            #                             message_header = f"{len(message):<{20}}".encode(
            #                             )
            #                             client_socket.send(
            #                                 online_clients[client_socket]['header'] + online_clients[client_socket]['data'] + message_header + message)
            #                             break
            #             else:
            #                 # FIXME uncomment debugging print before submitting
            #                 # print('FAIL: {} not online!'.format(username))
            #                 message = 'Error, {} not online!'.format(
            #                     username).encode()
            #                 message_header = f"{len(message):<{20}}".encode()
            #                 notified_socket.send(
            #                     user['header'] + user['data'] + message_header + message)
